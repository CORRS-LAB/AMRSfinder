...
cov.mod <- NULL
dat <- read.table(paste("bulk.sub.100bpWindows.txt.", num, sep = ""), header = T)
row_sd <- apply(dat[,-c(1:3)], 1, sd)
intput_dat <- dat[row_sd != 0, ]
  ...
  for(hh in c(1:50)){
       ...
       write.table(nfo,paste(paste("nfo.EWAS.100bpWindows.out.",DifAMRs,sep=""),num, sep= "."), row.names = F, sep = "\t", quote = F)
 } 
