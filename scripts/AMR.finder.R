...
### function to derive nfo$outputList
AMR.finder <- function(intput_dat, y,cov.mod){
  nfo<-list(maxdist=300,maxseg=-1,mincpgs=5,threads=1,mode=1,mtc=1,name="sample",trend=0.6,
          minNo=-1,minFactor=0.8,valley=0.7,minMethDist=0.1,randomseed=26061981)
  #nfo<-list(maxdist=300,maxseg=-1,mincpgs=5,threads=1,mode=1,mtc=1,name="sample",trend=0.5,
  #          minNo=-1,minFactor=0.8,valley=0.5,randomseed=26061981)
...
cov.mod <- NULL
intput_dat <- read.table(paste("bulk.sub.txt.", num, sep = ""), header = T)
  ...
  for(hh in c(1:50)){
       ...
       nfo$FDR <- p.adjust(nfo$p_value, method = "BH")
       write.table(nfo,paste(paste("nfo.out.",DifAMRs,sep=""),num, sep= "."), row.names = F, sep = "\t", quote = F)
 } 
